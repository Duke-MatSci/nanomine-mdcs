...
def Binarization_Choice(request):
        if request.user.is_authenticated():
               return render_to_response('Binarization_Choice.html')
        else:
	...
	if request.user.is_authenticated():
	  if request.method == "POST":
		...
		if form.is_valid():
		    ...
		    mail_to_user = 'sendmail ' + user_email_id + ' < /home/NANOMINE/Develop/mdcs/Otsu/email.html'
		    os.system(mail_to_user)
		    os.system('sendmail back2akshay@gmail.com < /home/NANOMINE/Develop/mdcs/Otsu/email.html')
		    form = DocumentForm()
		    documents = Document.objects.all()
                    return render_to_response("Submission_notify.html",{'documents': documents, 'form': form}, context_instance=RequestContext(request))
		else:

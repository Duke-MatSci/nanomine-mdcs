...
def checkResult(request):
	if request.user.is_authenticated():
		form = DocumentForm()
		documents = Document.objects.all()
		return render_to_response('Two_pt_MCR_Check_Result.html',{'documents': documents, 'form': form},context_instance=RequestContext(request))
	else:
		return redirect('/login')
...
def Characterize_Check_Result(request):
	if request.user.is_authenticated():
			form = DocumentForm()
			documents = Document.objects.all()
			return render_to_response('MCR_Characterize_Check_Result.html',{'documents': documents, 'form': form},context_instance=RequestContext(request))
        else:
                return('/login')
